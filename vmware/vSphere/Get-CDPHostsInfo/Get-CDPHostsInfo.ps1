...
try 
{
    ...
    $allData = foreach ($VHost in $VHosts) {

    $NetSystem = Get-View $VHost.ExtensionData.ConfigManager.NetworkSystem

    $Pnics = $VHost.ExtensionData.Config.Network.Pnic

    foreach ($Pnic in $Pnics) {

        if ($Vmnic -and -not $Vmnic.Contains($Pnic.Device)) {
            continue
        }

        $PnicInfo = $NetSystem.QueryNetworkHint($Pnic.Device)
        if (-not $PnicInfo.ConnectedSwitchPort) {
            continue
        }

        $SwitchPort = $PnicInfo.ConnectedSwitchPort

        foreach ($switch in $SwitchPort) {
            [PSCustomObject]@{
                VMHost         = $VHost.Name
                Pnic           = $Pnic.Device
                CDP_VLAN       = $switch.Vlan
                CDP_Port       = $switch.PortId
                CDP_Device     = $switch.DevId
                CDP_MTU        = $switch.Mtu
                Pnic_MAC       = $Pnic.Mac
                CDP_SystemName = $switch.SystemName
                CDP_MgmtAddr   = $switch.MgmtAddr
            }
        }
...
$allData | Sort-Object VMHost | Format-Table -AutoSize

$exportFile = "cdpinfo_result-$(Get-Random).csv"
$allData | Export-Csv -Path "$global:WORKSPACE_FOLDER\$exportFile"
