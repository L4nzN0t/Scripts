...
class Log {
    ...
    [void] LogAppend($logType, $task, $server, $user, $description) {
        ...
        $objLog = New-Object -TypeName psobject -Property $properties

        $stringLog = $objLog.Date + " UTC" + " [" + $objLog.Type + "] " + $objLog.Server + " " + $objLog.User + " - " + $objLog.Task + " - " + $objLog.Description
...
try {
    ...
    $clientLog.LogAppend("INFO", "ALL VCENTER CONNECTIONS", "LOCAL", $global:user, "$($global:DefaultVIServers.count) of $($vcenters.count) connected servers")
...
try 
{
    ...
    foreach ($esxhost in $hosts) {
        ...
        if ($esxhost.PowerState -ne 'PoweredOn') {
            continue
        }
        ...
        try { 
            ...
            $clientLog.LogAppend("INFO", "[$esxhost]", "LOCAL", $global:user, "TPM ENABLED: NOT FOUND")
            Write-Host "[INFO] [$esxhost] TPM ENABLED: NOT FOUND" -ForegroundColor Red
            
            $obj.EncryptionMode = "NOT FOUND"
            $obj.SecureBoot = "NOT FOUND"
            $obj.TPM_RecoveryID = "N/A"
            $obj.TPM_RecoveryKey = "N/A"
            continue
        }
        
        if ($tpmmode.Mode -eq "NONE") {
            $clientLog.LogAppend("INFO", "[$esxhost]", "LOCAL", $global:user, "TPM ENABLED: FALSE")
            Write-Host "[INFO] [$esxhost] TPM ENABLED: FALSE" -ForegroundColor Yellow
            
            $obj.EncryptionMode = $tpmmode.Mode
            $obj.SecureBoot = $tpmmode.RequireSecureBoot
            $obj.TPM_RecoveryID = "N/A"
            $obj.TPM_RecoveryKey = "N/A"
        } else {
            $clientLog.LogAppend("INFO", "[$esxhost]", "LOCAL", $global:user, "TPM ENABLED: TRUE")
            Write-Host "[INFO] [$esxhost] TPM ENABLED: TRUE" -ForegroundColor Green

            $tpmkey = $esxcli.system.settings.encryption.recovery.list.Invoke()

            $obj.EncryptionMode = $tpmmode.Mode
            $obj.SecureBoot = $tpmmode.RequireSecureBoot
            $obj.TPM_RecoveryID = $tpmkey.RecoveryID
            $obj.TPM_RecoveryKey = $tpmkey.Key
        }
        $allObj += $obj

    }

    $allObj | Sort-Object VMHost | Format-Table -AutoSize

    $exportFile = "recovery_keys_$(Get-Date -Format "MMddyyyy_hhmmss").csv"
    $allObj | Export-Csv -Path "$global:WORKSPACE_FOLDER\$exportFile"
    Write-Host ""
    Write-Host "Data exported to $global:WORKSPACE_FOLDER\$exportFile" -ForegroundColor Cyan
}
#################################################################################################
