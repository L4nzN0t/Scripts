...
$regex = "[\$@&\?\*\#\.\|\<\>\:\{\}\(\)\[\]\+\%\!\'\/]"
$renamedGPOs = $allGPOs | ForEach-Object { $_.DisplayName -replace $regex,"_" }

foreach ($gpo in $renamedGPOs) {
    Backup-GPO -Name $gpo -Path $BackupFolder -server $ServerFQDN -Domain $Domain
    Get-GPOReport –Name $gpo –ReportType HTML –Path “$ReportFolder\$($gpo).html” –server $ServerFQDN -Domain $Domain
    }
